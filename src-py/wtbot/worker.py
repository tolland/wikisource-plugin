...
def _upsert_page(session: Session, site: Site, remote: RemotePage) -> Page:
    ...
    if remote.content_model == "proofread-page" and remote.text:
        page.header, page.body, page.footer = _decompose_proofread_page(remote.text)
...
def _decompose_proofread_page(text: str) -> tuple[str, str, str]:
    """Split a ProofreadPage full text into (header, body, footer).

    ProofreadPage wraps sections in <div> pairs generated by the extension:
      <div class="pagetext"><div>HEADER</div>BODY<div>FOOTER</div></div>
    The outermost open/close divs delimit header and footer; everything between
    the first close and last open is the human-readable body.

    Returns empty strings for any part that cannot be parsed, rather than
    raising — callers store what they can.
    """
    # pywikibot uses p_open = re.compile(r'<div\b[^>]*>') and p_close = </div>
