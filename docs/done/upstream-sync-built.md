...
Completed pieces of the upstream-sync feature, kept for the record of what
they are and why they're shaped this way. The active work list is
`docs/todo/upstream-sync-TODO.md`; reasoning and rejected approaches are in
`docs/design/upstream-sync-discussion.md`; measured ProofreadPage hash
behaviour is in `docs/reference/proofread-page-sha1-discordance.md`.
...
- **Directional**, unlike everything under it. A pairing and a rung are
...
- **Addressed by title, not by a tracked work.** The case most in need of a
  ...
  came for.
- Pagination comes from the cache, which is `list=proofreadpagesinindex`
...
- **The §6 scan check runs first**, and its result is on the response
...
- Per page: the anchor revids, the ladder depth, and how far each side has
  moved. Where a pair cannot be matched the matcher's own reason is
  carried through rather than flattened to "no".
- **Three kinds of absence, distinguished.** No target index (whole work is
...
- **Output is a report.** No writes to either wiki, and none to the local
...
- **A work is not only its pages.** The `Index:` and the backing `File:`
...
- **"Could not check" is a question, not a verdict.** Both of those mean
...
- The file title comes from the index title (`Index:Foo.pdf` →
...
- **The verdicts come off the stored ladder, not off a comparison.** This
...
- `POST /links/propose { local, remote, index_title, remote_index_title? }`
...
- Never auto-confirm. `propose` writes nothing without `confirm`, and
...
- **Staging freezes the report.** The report is recomputed per load, and
...
## Testing (baseline, already in place)
...
- The copied-work fixture: copy a page across, link it, and confirm the two
  sides read as the same transcription under different attribution.
- Diverge one side and confirm the anchor falls behind the head — the link
