...
| [`pages-and-existence.md`](design/pages-and-existence.md) | wtbot model | in progress — step 1 and nearly all of step 2 landed: every address-keyed table (journal, commit, meta, annotations, pairings) is on `title`; `PromotionBatch` reduced to two ends; `IndexLink`/`IndexMeta` share keys; `Transclusion`/`FileMeta` dropped for redesign (§12) | **Next:** move the address columns (`namespace_key`, `dirty`, `fetch_status`) onto `title`. Directed, mirrored links deferred; per-direction tracking decided (§0). |
