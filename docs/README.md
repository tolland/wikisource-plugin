...
| [`pages-and-existence.md`](design/pages-and-existence.md) | wtbot model | in progress — steps 1 and most of 2 landed: journal, commit, meta, annotations on `title`; `PromotionBatch` reduced to two ends; `IndexLink` shares `PageLink`'s key | **Next:** move the address columns (`namespace_key`, `dirty`, `fetch_status`) onto `title`. Directed, mirrored links are deferred (§0). |
...
| [`ui-tests.md`](reference/ui-tests.md) | plugin + docker harness | first cut: startup, editor, tool-window smoke tests | How `./gradlew integrationTest` stands up the docker backend and drives the IDE; read before adding a UI test. |
