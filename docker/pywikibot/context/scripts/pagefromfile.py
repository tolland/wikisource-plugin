...
class PageFromFileRobot(SingleSiteBot, CurrentPageBot):
    ...
    def treat_page(self) -> None:
        ...
        comment_top = (
            comment + " - " + i18n.twtranslate(self.site, "pagefromfile-msg_top")
        )
        comment_bottom = (
            comment + " - " + i18n.twtranslate(self.site, "pagefromfile-msg_bottom")
        )
        ...
        if page.exists():
            ...
            if nocontent and (
                nocontent in pagecontents or nocontent.lower() in pagecontents
            ):
            ...
            if self.opt.append:
                ...
                pywikibot.info(
                    f"Page {title} already exists, appending on "
                    f"{self.opt.append[0]}!"
                )
        ...
        self.put_current(
            contents, summary=comment, minor=self.opt.minor, show_diff=self.opt.showdiff
        )
...
class PageFromFileReader(OptionHandler, GeneratorWrapper):
    ...
    def _make_regexes(self):
        ...
        title_regex = re.compile(
            re.escape(self.opt.titlestart) + "(.*?)" + re.escape(self.opt.titleend)
        )
